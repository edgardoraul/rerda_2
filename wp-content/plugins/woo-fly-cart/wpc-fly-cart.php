...
! defined( 'WOOFC_VERSION' ) && define( 'WOOFC_VERSION', '5.0.8' );
		...
		if ( ! class_exists( 'WPCleverWoofc' ) && class_exists( 'WC_Product' ) ) {
			class WPCleverWoofc {
				...
				function enqueue_scripts() {
					...
					if ( get_option( '_woofc_instant_checkout', 'no' ) === 'yes' ) {
					...
					wp_localize_script( 'woofc-frontend', 'woofc_vars', array(
							...
							'slick_params'        => apply_filters( 'woofc_slick_params', json_encode( array(
								'slidesToShow'   => 1,
								'slidesToScroll' => 1,
								'dots'           => true,
								'arrows'         => false,
                                    ...
                                    <table class="form-table">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <p style="color: #c9356e">This feature is available for Premium Version
                                                    only.</p>
                                            </td>
                                        </tr>
                                        <tr>
				...
				function get_cart_area() {
					...
					$related   = $cross_sells = $suggested_products = array();
					...
					if ( ! empty( $items ) ) {
						...
						if ( $suggested !== 'none' ) {
							...
							if ( ! empty( $suggested_products ) ) {
								...
								$cart_html .= '<div class="woofc-cross-sells-heading">' . self::localization( 'cross_sells', esc_html__( 'You may be interested in&hellip;', 'woo-fly-cart' ) ) . '</div>';
